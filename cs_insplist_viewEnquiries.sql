...
** @pservice_c = service code (optional)
...
** @psort_order = sort order (optional, default value is 'D')
**   D=do_date, P=postcode, W=ward, A=area
**   DD=do_date desc, PD=postcode desc, WD=ward desc, AD=area desc
**
** Returned
** Inspection list
**
...
** 17/03/2013  TW  Additional parameter @service_c
** 24/03/2013  TW  Additional parameter @sort_order
** 10/04/2013  TW  Include works orders
** 24/04/2013  TW  Include works orders parameterised
** 30/04/2013  TW  Reduced the number of sort options
** 01/05/2013  TW  System key holding default value of @months
** 22/05/2013  TW  Additional column ward_name; ward sort now by ward_name
...
BEGIN
	...
	DECLARE @errortext varchar(500),
		...
		@action_flag varchar,
		@do_date datetime,
		@sort_order varchar(2),
		@fcinstalled bit;


	SET @errornumber = '20111';
	...
	SET @sort_order = LTRIM(RTRIM(@psort_order));

	IF @UName = '' OR @UName IS NULL
	BEGIN
		SET @errornumber = '20112';
	...
	SELECT complaint_no
		...
		ORDER BY
			CASE WHEN @sort_order = 'D'  THEN do_date END,
			CASE WHEN @sort_order = 'D'  THEN end_time_h END,
			CASE WHEN @sort_order = 'D'  THEN end_time_m END,
			CASE WHEN @sort_order = 'D'  THEN site_name_1 END,
			CASE WHEN @sort_order = 'DD' THEN do_date END DESC,
			CASE WHEN @sort_order = 'DD' THEN end_time_h END DESC,
			CASE WHEN @sort_order = 'DD' THEN end_time_m END DESC,
			CASE WHEN @sort_order = 'DD' THEN site_name_1 END DESC,
			CASE WHEN @sort_order = 'P'  THEN postcode END,
			CASE WHEN @sort_order = 'P'  THEN site_name_1 END,
			CASE WHEN @sort_order = 'PD' THEN postcode END DESC,
			CASE WHEN @sort_order = 'PD' THEN site_name_1 END DESC,
			CASE WHEN @sort_order = 'W'  THEN ward_name END,
			CASE WHEN @sort_order = 'W'  THEN site_name_1 END,
			CASE WHEN @sort_order = 'WD' THEN ward_name END DESC,
			CASE WHEN @sort_order = 'WD' THEN site_name_1 END DESC,
			CASE WHEN @sort_order = 'A'  THEN pa_area END,
			CASE WHEN @sort_order = 'A'  THEN site_name_1 END,
			CASE WHEN @sort_order = 'AD' THEN pa_area END DESC,
			CASE WHEN @sort_order = 'AD' THEN site_name_1 END DESC;
