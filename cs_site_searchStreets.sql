/*****************************************************************************
** dbo.cs_site_searchStreets
** stored procedure
**
** Description
** Selects a list of site street records using postcode, building number,
...
**
*****************************************************************************/
IF OBJECT_ID (N'dbo.cs_site_searchStreets', N'P') IS NOT NULL
    DROP PROCEDURE dbo.cs_site_searchStreets;
GO
CREATE PROCEDURE dbo.cs_site_searchStreets
...
BEGIN
	...
	IF @use_supersites = 'Y'
	BEGIN
		SET @query = @query + ' AND ([site].site_ref LIKE ''%S'' OR [site].site_ref LIKE ''%G'')';
	END
	ELSE
	BEGIN
		SET @query = @query + ' AND [site].site_c IN (''S'', ''G'')';
	END

	SET @query = @query + ' ORDER BY site_sort, site_name_1;';

	BEGIN TRY
		EXECUTE (@query);
	END TRY
	BEGIN CATCH
		SET @errornumber = '13601';
