/*****************************************************************************
** dbo.cs_comp_getEnforcementDetails
** stored procedure
**
** Description
** Selects Enforcement data for a given complaint_no
**
** Parameters
** @complaint_no = complaint number
**
** Returned
** Result set of Enforcement data
**
** Notes
**
** History
...
**
*****************************************************************************/
IF OBJECT_ID (N'dbo.cs_comp_getEnforcementDetails', N'P') IS NOT NULL
    DROP PROCEDURE dbo.cs_comp_getEnforcementDetails;
GO
CREATE PROCEDURE dbo.cs_comp_getEnforcementDetails
...
BEGIN
	...
	SELECT 
		...
		action_days_remaining = 
			...
			CASE 
				WHEN DATEDIFF(day, CONVERT(datetime, CONVERT(date, GETDATE())), cs_comp_viewEnfList.do_date) > 0 THEN DATEDIFF(day, CONVERT(datetime, CONVERT(date, CONVERT(datetime, CONVERT(date, GETDATE())))), cs_comp_viewEnfList.do_date)
				ELSE 0
			END

		FROM cs_comp_viewEnfList, cs_comp_viewHistory, site_detail
		WHERE cs_comp_viewEnfList.complaint_no = @complaint_no 
			AND (cs_comp_viewEnfList.[state] = 'A' OR cs_comp_viewEnfList.[state] = 'P')
			...
			AND site_detail.site_ref = cs_comp_viewHistory.site_ref
