/*****************************************************************************
** dbo.cs_comp_getMyHway
** stored procedure
**
** Description
** Selects a set of current comp records for Highways service and a given officer
**
** Parameters
** @username = user name
**
...
**
*****************************************************************************/
IF OBJECT_ID (N'dbo.cs_comp_getMyHway', N'P') IS NOT NULL
    DROP PROCEDURE dbo.cs_comp_getMyHway;
GO
CREATE PROCEDURE dbo.cs_comp_getMyHway
...
BEGIN
	...
	SELECT 
		...
		WHERE cs_comp_viewHistory.service_c = (SELECT c_field FROM keys WHERE keyname = 'HWAY_SERVICE' AND service_c = 'ALL')
