/*****************************************************************************
** dbo.cs_comp_getMyGraffiti
** stored procedure
**
** Description
** Selects a set of current comp records for Graffiti service and a given officer
**
** Parameters
** @username = user name
**
...
**
*****************************************************************************/
IF OBJECT_ID (N'dbo.cs_comp_getMyGraffiti', N'P') IS NOT NULL
    DROP PROCEDURE dbo.cs_comp_getMyGraffiti;
GO
CREATE PROCEDURE dbo.cs_comp_getMyGraffiti
...
BEGIN
	...
	SELECT 
		...
		WHERE cs_comp_viewHistory.service_c = (SELECT c_field FROM keys WHERE keyname = 'ERT_SERVICE' AND service_c = 'ALL')
