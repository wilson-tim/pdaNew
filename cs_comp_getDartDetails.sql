/*****************************************************************************
** dbo.cs_comp_getDartDetails
** stored procedure
**
** Description
** Selects DART customer care data for a given complaint_no
**
** Parameters
...
**
*****************************************************************************/
IF OBJECT_ID (N'dbo.cs_comp_getDartDetails', N'P') IS NOT NULL
    DROP PROCEDURE dbo.cs_comp_getDartDetails;
GO
CREATE PROCEDURE dbo.cs_comp_getDartDetails
...
BEGIN
	...
	SELECT 
		...
		WHERE cs_comp_viewHistory.complaint_no = @complaint_no
			AND cs_comp_viewHistory.service_c = (SELECT c_field FROM keys WHERE keyname = 'DART_SERVICE' AND service_c = 'ALL')
