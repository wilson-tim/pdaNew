/*****************************************************************************
** dbo.csis_woh_getChangedWorksOrders
** stored procedure
**
** Description
** Selects changed works orders
**
** Parameters
** @user_name    = user name
...
**
*****************************************************************************/
IF OBJECT_ID (N'dbo.csis_woh_getChangedWorksOrders', N'P') IS NOT NULL
    DROP PROCEDURE dbo.csis_woh_getChangedWorksOrders;
GO
CREATE PROCEDURE dbo.csis_woh_getChangedWorksOrders
...
BEGIN
	...
	SELECT
		...
		WHERE 
			...
			/* Updated works order status history or text since last data transfer */
			AND (
				((
				SELECT COUNT(*) FROM wo_stat_hist
				WHERE wo_stat_hist.wo_ref = wo_h.wo_ref
					AND wo_stat_hist.wo_suffix = wo_h.wo_suffix
					AND
					(DATEADD(minute, CAST(ISNULL(wo_stat_hist.change_time_m, 0) AS integer)+1,
					DATEADD(hour,   CAST(ISNULL(wo_stat_hist.change_time_h, 0) AS integer),
					DATEADD(day,    DATEPART(day,   wo_stat_hist.change_date) - 1, 
					DATEADD(month,  DATEPART(month, wo_stat_hist.change_date) - 1, 
					DATEADD(year,   DATEPART(year,  wo_stat_hist.change_date) - 1900, 0)))))) >= itl.transfer_datetime
					) > 0)
				OR
				((SELECT COUNT(*) FROM wo_h_txt
					WHERE wo_h_txt.wo_ref = wo_h.wo_ref
						AND wo_h_txt.wo_suffix = wo_h.wo_suffix
						AND
						(DATEADD(minute, CAST(ISNULL(wo_h_txt.time_entered_m, 0) AS integer)+1,
						DATEADD(hour,   CAST(ISNULL(wo_h_txt.time_entered_h, 0) AS integer),
						DATEADD(day,    DATEPART(day,   wo_h_txt.doa) - 1, 
						DATEADD(month,  DATEPART(month, wo_h_txt.doa) - 1, 
						DATEADD(year,   DATEPART(year,  wo_h_txt.doa) - 1900, 0)))))) >= itl.transfer_datetime
						) > 0)
				)
			AND (@FromDateTime IS NULL
				OR
				(DATEADD(minute, CAST(ISNULL(wo_h.expected_time_m, 0) AS integer)+1,
