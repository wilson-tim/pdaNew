/*****************************************************************************
** dbo.cs_comp_createRecordAv
** stored procedure
**
** Description
** Create a customer care record, etc. for service type AV Abandoned Vehicles
**
** Parameters
** @xmlCompAV = XML structure containing comp (AV) record
**
** Returned
...
**
** It is only possible to add AV enquiry notes via cs_comp_updateRecordAV
**
** History
** 20/02/2013  TW  New
...
**
*****************************************************************************/
IF OBJECT_ID (N'dbo.cs_comp_createRecordAv', N'P') IS NOT NULL
    DROP PROCEDURE dbo.cs_comp_createRecordAv;
GO
CREATE PROCEDURE dbo.cs_comp_createRecordAv
...
BEGIN
	...
	DECLARE @result integer
		...
		,@av_multiple_comps varchar(1)
		,@av_comp_count integer
		,@av_complaint_no integer
	...
	SET @car_id = UPPER(LTRIM(RTRIM(@car_id)));
	SET @vin = UPPER(LTRIM(RTRIM(@vin)));
	SET @road_fund_flag = UPPER(LTRIM(RTRIM(@road_fund_flag)));
	...
	BEGIN
		...
		BEGIN
			...
			SET @errortext = 'service_c is not defined in system key AV_SERVICE';
			GOTO errorexit;
		END
	END
	...
	BEGIN
		...
		IF @item_ref = '' OR @item_ref IS NULL
		BEGIN
			SET @errornumber = '10808';
			SET @errortext = 'item_ref is not defined in system key AV_ITEM';
			GOTO errorexit;
		END
	END
	...
	BEGIN
		SELECT @feature_ref = c_field,
			@feature_desc = keydesc
			FROM keys
			WHERE keyname = 'AV_ITEM';

		IF @feature_ref = '' OR @feature_ref IS NULL
		BEGIN
			SET @errornumber = '20130';
			SET @errortext = 'feature_ref is not defined in system key AV_ITEM';
			GOTO errorexit;
		END
	END
	...
	BEGIN
		SELECT @contract_ref = c_field, 
               @contract_name = keydesc
          FROM keys
         WHERE keyname = 'AV_CONTRACT';

		IF @contract_ref = '' OR @contract_ref IS NULL
		BEGIN
			SET @errornumber = '10818';
			SET @errortext = 'contract_ref is not defined in system key AV_CONTRACT';
			GOTO errorexit;
		END
	END
	...
	BEGIN
		...
		IF @comp_code = '' OR @comp_code IS NULL
		BEGIN
			SET @errornumber = '10809';
			SET @errortext = 'comp_code is not defined in system key AV_COMP_CODE';
			GOTO errorexit;
		END
	END
	...
	/* @car_id validation */
	IF @car_id = '' OR @car_id IS NULL
	BEGIN
		SET @car_id = 'NOPLATES';
	END
	ELSE
	BEGIN
		/* Remove spaces from @car_id */
		SET @car_id = REPLACE(@car_id, ' ', '')
	END
	SET @av_comp_count     = 0;
	SET @av_multiple_comps = UPPER(LTRIM(RTRIM(dbo.cs_keys_getCField('ALL', 'AV MULTIPLE COMPS'))));
	IF @av_multiple_comps <> 'Y'
	BEGIN
		SELECT @av_comp_count = COUNT(*)
			FROM comp_av
			WHERE car_id = @car_id;
	END
	ELSE
	...
	BEGIN
		...
		BEGIN
			IF @av_multiple_comps <> 'Y'
			BEGIN
				SELECT @av_complaint_no = complaint_no
					FROM comp_av
					WHERE car_id = @car_id;
			END
			...
			IF @av_multiple_comps <> 'Y'
			BEGIN
				SET @errornumber = '20174';
				SET @errortext   = 'The vehicle ' + @car_id + ' exists in the LIVE system (Ref: ' + LTRIM(RTRIM(STR(@av_complaint_no)));
				GOTO errorexit;
			END
		END
		ELSE
		BEGIN
			SET @errornumber = '20175';
			SET @errortext   = 'Multiple records for vehicle ' + @car_id + ' exist in the LIVE system. Contact the System Administrator';
			GOTO errorexit;
		END		
	END
	...
	IF @reccount <> 1
	BEGIN
		SET @errornumber = '20304';
		SET @errortext = LTRIM(RTRIM(STR(@make_ref))) + ' is not a valid make reference';
		GOTO errorexit;
	END

	/* @model_ref validation */
	IF @model_ref IS NULL
	BEGIN
		SET @errornumber = '10812';
		SET @errortext = 'model_ref is required';
		GOTO errorexit;
	END
	SELECT @reccount = COUNT(*)
		FROM models
		WHERE make_ref = @make_ref
			AND model_ref = @model_ref;
	IF @reccount <> 1
	BEGIN
		SET @errornumber = '20305';
		SET @errortext = LTRIM(RTRIM(STR(@model_ref))) + ' is not a valid model reference';
		GOTO errorexit;
	END

	/* @colour_ref validation */
	IF @colour_ref IS NULL
	BEGIN
		SET @errornumber = '20303';
		SET @errortext = 'colour_ref is required';
		GOTO errorexit;
	END
	SELECT @reccount = COUNT(*)
		FROM colour
		WHERE colour_ref = @colour_ref;
	IF @reccount <> 1
	BEGIN
		SET @errornumber = '20306';
		SET @errortext = LTRIM(RTRIM(STR(@colour_ref))) + ' is not a valid colour reference';
		GOTO errorexit;
	END
	...
	IF @road_fund_valid IS NULL
	BEGIN
		SET @road_fund_flag = 'N';
	END
	ELSE
	BEGIN
		SET @road_fund_flag = 'Y';
		SET @road_fund_valid = CONVERT(datetime, CONVERT(date, @road_fund_valid));
	END
